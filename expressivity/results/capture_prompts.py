"""Capture the rendered extraction prompts for one hearsay instance.

Renders the exact prompts the framework sends (same format calls as
evaluator.extract_terms / extract_predicates), runs the real pipeline on one
short hearsay test instance, and writes the prompts, the structured model
responses, and the resulting ABox to expressivity/results/. The output feeds
the representative-prompt appendix in the paper.

Run: uv run expressivity/capture_prompts.py
"""
